...
def init_session_state():
    ...
    if 'openai_messages' not in st.session_state:
        st.session_state.openai_messages = [
            ...
            ### 質問
            下記の質問を順番に質問してください。参加者は5チーム（チーム１からチーム５）が交代で答えます
            1チームが答えたら、次のチームに当てます
            正解するまで次の問題にいきません。
            間違えたらヒントを出します。しかし正解自体は教えません。

            質問：鎌倉幕府を開いた源頼朝（みなもとのよりとも）が征夷大将軍（せいいたいしょうぐん）に任命されたのは何年や？
            答え：1192年
            ...
            質問：西鉄（にしてつ）久留米から附設高校前に停まるバスの行き先番号は何番や？
            答え：２番と７番（両方答えないとダメ）
            
            質問：イブニング附設で提供されていた食事は？
            答え：豚汁
            これはヒントなしで
            ...
            ### 最後の会話
            参加者が10問とも正解したら、「まじか！全問正解かい」というコメントをしてください。
